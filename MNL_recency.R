...
nlevels <- c(6, 3,4, 2,3)
nattributes <- 5
beta0 <- c(0.2141, -0.0135, -0.0067, 0.0395, -0.1169, -0.0180, -0.0247, -0.0308, 0.0174, 0.0389, 0.0417, -0.3, 0)
nparam <- length(beta0)
nresps <- 150

## Designs
# naive design
X_data <- readMat(".../design/X_naive.mat")
X <- X_data$X.naive
...
beta_MNL_naive_recency <- data.frame()
b <- 0
...
while (b < ndraws) {
  ...
  while (!estimation_success) {
    ...
    tryCatch({
      fit_MNL_with_order_naive_recency <- mlogit(chosen ~ V1 + V2 + V3 + V4 + V5 + V6 + 
                                                   V7 + V8 + V9 + V10 + V11 + V12 + V13 | 0, data = data_z)
      
      beta_MNL_naive_recency <- rbind(beta_MNL_naive_recency, as.data.frame(t(fit_MNL_with_order_naive_recency[["coefficients"]][1:13])))
...
# Random order
beta_MNL_random_recency <- data.frame()
b <- 0
...
while (b < ndraws) {
  ...
  while (!estimation_success) {
    ...
    # Try model estimation
    tryCatch({
      fit_MNL_with_order_random_recency <- mlogit(chosen ~ V1 + V2 + V3 + V4 + V5 + V6 + 
                                                    V7 + V8 + V9 + V10 + V11 + V12 + V13 | 0, data = data_z)
      
      beta_MNL_random_recency <- rbind(beta_MNL_random_recency, as.data.frame(t(fit_MNL_with_order_random_recency[["coefficients"]][1:13])))
...
# Save results
write.csv(beta_MNL_naive_recency, ".../beta_MNL_naive_recency.csv", row.names = FALSE)
write.csv(beta_MNL_random_recency, ".../beta_MNL_random_recency.csv", row.names = FALSE)
