...
nlevels <- c(6, 3,4, 2,3)
nattributes <- 5
mu <- c(0.2141, -0.0135, -0.0067, 0.0395, -0.1169, -0.0180, -0.0247, -0.0308, 0.0174, 0.0389, 0.0417, -0.15, 0.3)
sigma <- c(0.2264,0.0908,0.0030,0.2226,0.3111,0.1339,0.0180,0.1251,0.1828,0.1914,0.2668,0.3,0.3)
nparam <- length(mu)
nresps <- 150
...
beta_PML_naive_Central <- data.frame()
sd_PML_naive_Central <- data.frame()
b <- 0
...
while (b < ndraws) {
  ...
  while (!estimation_success) {
    ...
    tryCatch({
      fit_PML_with_order_naive_Central <- mlogit(chosen ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13|0, 
                                                 data = data_z,
                                                 rpar = c(V1 = "n", V2 = "n", V3 = "n", V4 = "n", V5 = "n", V6 = "n", V7 = "n", V8 = "n", V9 = "n", V10 = "n", V11 = "n", V12 = "n", V13 = "n"), 
                                                 panel = TRUE, R = 1000,halton = list(dim = 13))
      
      beta_PML_naive_Central <- rbind(beta_PML_naive_Central, as.data.frame(t(fit_PML_with_order_naive_Central[["coefficients"]][1:13])))
      sd_PML_naive_Central <- rbind(sd_PML_naive_Central, as.data.frame(t(fit_PML_with_order_naive_Central[["coefficients"]][14:26])))
...
beta_PML_random_Central <- data.frame()
sd_PML_random_Central <- data.frame()
...
while (b < ndraws) {
  ...
  while (!estimation_success) {
    ...
    # Try model estimation
    tryCatch({
      fit_PML_with_order_random_Central <- mlogit(chosen ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13|0, 
                                                  data = data_z,
                                                  rpar = c(V1 = "n", V2 = "n", V3 = "n", V4 = "n", V5 = "n", V6 = "n", V7 = "n", V8 = "n", V9 = "n", V10 = "n", V11 = "n", V12 = "n", V13 = "n"), 
                                                  panel = TRUE, R = 1000,halton = list(dim = 13))
      
      beta_PML_random_Central <- rbind(beta_PML_random_Central, as.data.frame(t(fit_PML_with_order_random_Central[["coefficients"]][1:13])))
      sd_PML_random_Central <- rbind(sd_PML_random_Central, as.data.frame(t(fit_PML_with_order_random_Central[["coefficients"]][14:26])))
      estimation_success <- TRUE
      duration <- Sys.time() - start_time
      cat('draw:', b+1, 'duration:', duration, "\n")
    }, error = function(e) {
...
write.csv(beta_PML_random_Central, ".../beta_PML_random_central.csv", row.names = FALSE)
write.csv(sd_PML_random_Central, ".../sd_PML_random_central.csv", row.names = FALSE)

write.csv(beta_PML_naive_Central, ".../beta_PML_naive_Central.csv", row.names = FALSE)
write.csv(sd_PML_naive_Central, ".../sd_PML_naive_Central.csv", row.names = FALSE)
