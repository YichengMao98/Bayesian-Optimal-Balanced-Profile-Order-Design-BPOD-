...
beta_MNL_naive_primacy <- data.frame()
b <- 0
...
while (b < ndraws) {
  ...
  while (!estimation_success) {
    ...
    tryCatch({
      fit_MNL_with_order_naive_primacy <- mlogit(chosen ~ V1 + V2 + V3 + V4 + V5 + V6 + 
                                                   V7 + V8 + V9 + V10 + V11 + V12 + V13 | 0, data = data_z)
      
      beta_MNL_naive_primacy <- rbind(beta_MNL_naive_primacy, as.data.frame(t(fit_MNL_with_order_naive_primacy[["coefficients"]][1:13])))
...
# Random order
beta_MNL_random_primacy <- data.frame()
b <- 0
...
while (b < ndraws) {
  ...
  while (!estimation_success) {
    ...
    # Try model estimation
    tryCatch({
      fit_MNL_with_order_random_primacy <- mlogit(chosen ~ V1 + V2 + V3 + V4 + V5 + V6 + 
                                                    V7 + V8 + V9 + V10 + V11 + V12 + V13 | 0, data = data_z)
      
      beta_MNL_random_primacy <- rbind(beta_MNL_random_primacy, as.data.frame(t(fit_MNL_with_order_random_primacy[["coefficients"]][1:13])))
...
# Save results
write.csv(beta_MNL_naive_primacy, ".../beta_MNL_naive_primacy.csv", row.names = FALSE)
write.csv(beta_MNL_random_primacy, ".../beta_MNL_random_primacy.csv", row.names = FALSE)
