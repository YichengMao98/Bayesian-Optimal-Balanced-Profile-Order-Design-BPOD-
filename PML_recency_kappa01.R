...
beta_PML_balance_recency <- data.frame()
sd_PML_balance_recency <- data.frame()
b <- 0
...
while (b < ndraws) {
  ...
  while (!estimation_success) {
    ...
    tryCatch({
      fit_PML_with_order_balance_recency <- mlogit(chosen ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13|0, 
                                                   data = data_z,
                                                   rpar = c(V1 = "n", V2 = "n", V3 = "n", V4 = "n", V5 = "n", V6 = "n", V7 = "n", V8 = "n", V9 = "n", V10 = "n", V11 = "n", V12 = "n", V13 = "n"), 
                                                   panel = TRUE, R = 1000,halton = list(dim = 13))
      
      beta_PML_balance_recency <- rbind(beta_PML_balance_recency, as.data.frame(t(fit_PML_with_order_balance_recency[["coefficients"]][1:13])))
      sd_PML_balance_recency <- rbind(sd_PML_balance_recency, as.data.frame(t(fit_PML_with_order_balance_recency[["coefficients"]][14:26])))
...
beta_PML_random_recency <- data.frame()
sd_PML_random_recency <- data.frame()
...
while (b < ndraws) {
  ...
  while (!estimation_success) {
    ...
    # Try model estimation
    tryCatch({
      fit_PML_with_order_random_recency <- mlogit(chosen ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13|0, 
                                                  data = data_z,
                                                  rpar = c(V1 = "n", V2 = "n", V3 = "n", V4 = "n", V5 = "n", V6 = "n", V7 = "n", V8 = "n", V9 = "n", V10 = "n", V11 = "n", V12 = "n", V13 = "n"), 
                                                  panel = TRUE, R = 1000,halton = list(dim = 13))
      
      beta_PML_random_recency <- rbind(beta_PML_random_recency, as.data.frame(t(fit_PML_with_order_random_recency[["coefficients"]][1:13])))
      sd_PML_random_recency <- rbind(sd_PML_random_recency, as.data.frame(t(fit_PML_with_order_random_recency[["coefficients"]][14:26])))
      estimation_success <- TRUE
      duration <- Sys.time() - start_time
      cat('draw:', b+1, 'duration:', duration, "\n")
    }, error = function(e) {
...
write.csv(beta_PML_balance_recency, ".../beta_PML_balance_recency.csv", row.names = FALSE)
write.csv(beta_PML_random_recency, ".../beta_PML_random_recency.csv", row.names = FALSE)

write.csv(sd_PML_balance_recency, ".../sd_PML_balance_recency.csv", row.names = FALSE)
write.csv(sd_PML_random_recency, ".../sd_PML_random_recency.csv", row.names = FALSE)
